...
def load_tasks():
  ...
  if not os.path.exists(TASKS_FILE):
    return []
  with open(TASKS_FILE, 'r') as file: #permite leer
    tasks = file.readlines()
  return [task.strip() for task in tasks] #

def save_tasks(tasks):
  """Guardar las tareas en el archivo"""
  with open(TASKS_FILE, 'w') as file: #abrir en modo escritura
    for task in tasks:
      file.write(task + '\n') #escriba al final
...
def add_task(task):
  ...
  tasks = load_tasks()
  tasks.append(task) #agregar la tarea
  save_tasks(tasks)
...
def view_tasks():
  ...
  for i, task in enumerate(tasks, 1): #itera en forma de lista el enumerate
    print(f'{i}, {task}')
...
def  update_task(task_number, new_task):
  ...
  save_tasks(tasks)
  print(f'Tarea {task_number} actualizada a "{new_task}".') #agregar simples al inicio
